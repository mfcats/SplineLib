...
namespace spl {
template<int DIM>
class IGESBSplineGenerator : public BSplineGenerator<DIM> {
 public:
  explicit IGESBSplineGenerator(std::string filename) : filename_(std::move(filename)) {}
 ...
 private:
  void ReadParameterData(const std::vector<double> &parameterData) {
    if (DIM == 1) {
      if (parameterData[0] == 126) {
        ...
        throw std::runtime_error("You are trying to read a 2D spline with a 3D iges reader.");
      }
    } else if (DIM == 2) {
      if (parameterData[0] == 128) {
        Read2D(parameterData);
      } else {
        throw std::runtime_error("You are trying to read a 1D spline with a 2D iges reader.");
      }
    }
  }
  ...
  void Read1D(const std::vector<double> &parameterData) {
    ...
    if (parameterData[5] == 1) {
      ...
      throw std::runtime_error("You are trying to read a NURBS with the B-Spline spline generator.");
    }
  }
