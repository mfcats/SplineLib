...
ParameterSpace::ParameterSpace(const baf::KnotVector &knot_vector, int degree) : degree_(degree),
...
baf::KnotVector ParameterSpace::knot_vector() const {
  return knot_vector_;
}

std::vector<std::unique_ptr<baf::BasisFunction>>::const_iterator ParameterSpace::GetFirstNonZeroKnot(
