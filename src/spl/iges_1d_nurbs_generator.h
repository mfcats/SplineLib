...
#ifndef SPLINELIB_IGES_1D_NURBS_GENERATOR_H
#define SPLINELIB_IGES_1D_NURBS_GENERATOR_H
 ...
 private:
  void Read1D(const std::vector<double> &parameterData) {
    if ((parameterData[0] == 126) && (parameterData[5] == 0)) {
      ...
      this->physical_space_ptr = std::make_shared<WeightedPhysicalSpace<1>>(control_points, weights, number_of_points);
...
#endif //SPLINELIB_IGES_1D_NURBS_GENERATOR_H
