...
class A2DIRITReader : public Test, public A2DBSplineForIRIT, public A2DNURBSForIRIT {
 public:
  A2DIRITReader() : irit_reader(std::make_unique<io::IRITReader>()) {}
...
TEST_F(A2DIRITReader, ReturnsSameValuesAsGivenSplines) {  // NOLINT
  ...
  ASSERT_THAT(std::any_cast<std::shared_ptr<spl::NURBS<2>>>(spline_vector[4])->Evaluate({ParamCoord{0.123}}, {0})[0],
              DoubleEq(nurbs_->Evaluate({ParamCoord{0.123}}, {0})[0]));
  ASSERT_THAT(std::any_cast<std::shared_ptr<spl::NURBS<2>>>(spline_vector[4])->Evaluate({ParamCoord{0.123}}, {1})[0],
