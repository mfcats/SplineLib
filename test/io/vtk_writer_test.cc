...
class AVTKWriter
    : public Test, public A1DBNURBSForVTKWriter, public A2DBNURBSForVTKWriter, public A3DBNURBSForVTKWriter {  // NOLINT
 public:
  AVTKWriter() : vtk_writer_(std::make_unique<io::VTKWriter>()) {}
...
TEST_F(AVTKWriter, CreatesVTKFile) {  // NOLINT
  ...
  std::vector<std::vector<int>> scattering = {{10}, {20, 20}, {4, 3, 2}};
  vtk_writer_->WriteFile({nurbs_1d_any, nurbs_2d_any, nurbs_3d_any}, "splines.vtk", scattering);
...
class A4DBNURBSVTKWriter : public Test {  // NOLINT
 public:
  A4DBNURBSVTKWriter() : vtk_writer_(std::make_unique<io::VTKWriter>()) {
...
TEST_F(A4DBNURBSVTKWriter, AddSplineThrowsFor4DNURBS) {  // NOLINT
  std::any nurbs_4d_any = std::make_any<std::shared_ptr<spl::NURBS<4>>>(nurbs_4d_);
  ASSERT_THROW(vtk_writer_->WriteFile({nurbs_4d_any}, "4d_spline.vtk", {{4, 3, 2, 1}}), std::runtime_error);
