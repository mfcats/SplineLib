...
class Random1DBSplineForKnotInsertionAndRemoval : public Test {  // NOLINT
 public:
  Random1DBSplineForKnotInsertionAndRemoval() :
      param_coord_(ParamCoord{util::Random::GetUniformRandom<double>(0.0, 1.0)}) {
    ...
    removal_spline.RemoveKnot(param_coord_, 0, 1e-15);
  ...
  for (int point = 0; point < original_->GetNumberOfControlPoints(); ++point) {
    for (int dimension = 0; dimension < original_->GetDimension(); ++dimension) {
      ASSERT_THAT(after_removal_->GetControlPoint({point}, dimension),
                  DoubleNear(original_->GetControlPoint({point}, dimension), 1e-12));
    }
  }
}
...
TEST_F(Random1DNURBSForKnotInsertionAndRemoval, DoesNotChangePhysicallyAfterKnotRemoval) {  // NOLINT
  for (int point = 0; point < original_->GetNumberOfControlPoints(); ++point) {
    for (int dimension = 0; dimension < original_->GetDimension(); ++dimension) {
      ASSERT_THAT(after_removal_->GetControlPoint({point}, dimension),
                  DoubleNear(original_->GetControlPoint({point}, dimension), 1e-12));
    }
    ASSERT_THAT(after_removal_->GetWeight({point}), DoubleNear(original_->GetWeight({point}), 1e-12));
  }
}
...
class Random2DBSplineForKnotInsertionAndRemoval : public Test {  // NOLINT
 public:
  Random2DBSplineForKnotInsertionAndRemoval() : param_coord_{
      ParamCoord{util::Random::GetUniformRandom<double>(0.0, 1.0)},
      ParamCoord{util::Random::GetUniformRandom<double>(0.0, 1.0)},
      ParamCoord{util::Random::GetUniformRandom<double>(0.0, 1.0)}} {
    std::array<ParamCoord, 2> limits = {ParamCoord{0.0}, ParamCoord{1.0}};
    ...
    removal_spline.RemoveKnot(param_coord_[0], 0, 1e-12);
    removal_spline.RemoveKnot(param_coord_[1], 1, 1e-12);
    removal_spline.RemoveKnot(param_coord_[2], 1, 1e-12);
...
TEST_F(Random2DBSplineForKnotInsertionAndRemoval, DoesNotChangeGeometricallyAfterKnotInsertion) {  // NOLINT
  for (int evaluated_point = 0; evaluated_point < 25; ++evaluated_point) {
    std::array<ParamCoord, 2> param_coord{ParamCoord{util::Random::GetUniformRandom<double>(0.0, 1.0)},
                                          ParamCoord{util::Random::GetUniformRandom<double>(0.0, 1.0)}};
    for (int j = 0; j < original_->GetDimension(); ++j) {
      ASSERT_THAT(after_insertion_->Evaluate(param_coord, {j})[0],
                  DoubleNear(original_->Evaluate(param_coord, {j})[0], 1e-10));
...
TEST_F(Random2DBSplineForKnotInsertionAndRemoval, DoesNotChangePhysicallyAfterKnotRemoval) {  // NOLINT
  for (int point = 0; point < original_->GetNumberOfControlPoints(); ++point) {
    for (int dimension = 0; dimension < original_->GetDimension(); ++dimension) {
      ASSERT_THAT(after_removal_->GetControlPoint({point}, dimension),
                  DoubleNear(original_->GetControlPoint({point}, dimension), 1e-12));
...
class Random2DNURBSForKnotInsertionAndRemoval : public Test {  // NOLINT
 public:
  Random2DNURBSForKnotInsertionAndRemoval() : param_coord_{
    ...
    removal_spline.RemoveKnot(param_coord_[0], 0, 1e-12);
    removal_spline.RemoveKnot(param_coord_[1], 0, 1e-12);
    removal_spline.RemoveKnot(param_coord_[2], 1, 1e-12);
...
TEST_F(Random2DNURBSForKnotInsertionAndRemoval, HasOneLessControlPointPerDirectionAfterKnotRemoval) {  // NOLINT
  ASSERT_THAT(after_removal_->GetPointsPerDirection()[0], after_insertion_->GetPointsPerDirection()[0] - 2);
  ASSERT_THAT(after_removal_->GetPointsPerDirection()[1], after_insertion_->GetPointsPerDirection()[1] - 1);
}

TEST_F(Random2DNURBSForKnotInsertionAndRemoval, DoesNotChangePhysicallyAfterKnotRemoval) {  // NOLINT
  for (int point = 0; point < original_->GetNumberOfControlPoints(); ++point) {
    for (int dimension = 0; dimension < original_->GetDimension(); ++dimension) {
      ASSERT_THAT(after_removal_->GetControlPoint({point}, dimension),
                  DoubleNear(original_->GetControlPoint({point}, dimension), 1e-12));
...
class Random3DBSplineForKnotInsertionAndRemoval : public Test {  // NOLINT
 public:
  Random3DBSplineForKnotInsertionAndRemoval() : param_coord_{
    ...
    removal_spline.RemoveKnot(param_coord_[0], 0, 1e-15);
    removal_spline.RemoveKnot(param_coord_[1], 1, 1e-15);
    removal_spline.RemoveKnot(param_coord_[2], 2, 1e-15);
...
TEST_F(Random3DBSplineForKnotInsertionAndRemoval, DoesNotChangeGeometricallyAfterKnotInsertion) {  // NOLINT
  for (int evaluated_point = 0; evaluated_point < 10; ++evaluated_point) {
    std::array<ParamCoord, 3> param_coord{ParamCoord{util::Random::GetUniformRandom<double>(0.0, 1.0)},
                                          ParamCoord{util::Random::GetUniformRandom<double>(0.0, 1.0)},
                                          ParamCoord{util::Random::GetUniformRandom<double>(0.0, 1.0)}};
    for (int j = 0; j < original_->GetDimension(); ++j) {
      ASSERT_THAT(after_insertion_->Evaluate(param_coord, {j})[0],
...
TEST_F(Random3DBSplineForKnotInsertionAndRemoval, DoesNotChangePhysicallyAfterKnotRemoval) {  // NOLINT
  for (int point = 0; point < original_->GetNumberOfControlPoints(); ++point) {
    for (int dimension = 0; dimension < original_->GetDimension(); ++dimension) {
      ASSERT_THAT(after_removal_->GetControlPoint({point}, dimension),
                  DoubleNear(original_->GetControlPoint({point}, dimension), 1e-12));
...
TEST_F(Random3DNURBSForKnotInsertionAndRemoval, DoesNotChangePhysicallyAfterKnotRemoval) {  // NOLINT
  for (int point = 0; point < original_->GetNumberOfControlPoints(); ++point) {
    for (int dimension = 0; dimension < original_->GetDimension(); ++dimension) {
      ASSERT_THAT(after_removal_->GetControlPoint({point}, dimension),
                  DoubleNear(original_->GetControlPoint({point}, dimension), 1e-12));
